...
Decay like $1/t$ is the blowup rate with its sign reversed, which is what the
same profile family produces when the singularity lies in the past. The window
is open rather than a separatrix, since both $c=0.50$ and $c=0.52$ fail to blow
up while $0.48$ and $0.56$ do, so the model admits global decaying solutions
from an open set of sign changing data. The window is narrow, roughly
$0.49<c<0.55$, and we do not resolve its interior.
