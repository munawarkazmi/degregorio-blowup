...
above a critical advection, a blowup whose spatial scale neither shrinks nor
expands, $\omega = f(x)/(T-t)$. This frozen profile was found numerically by
Lushnikov, Silantyev and Siegel, and Chen proved its existence and nonlinear
stability near $a=1$. Its Fourier spectrum decays algebraically,
$\abs{\hat f_k}\sim k^{-\beta}$, with $\beta$ obtained hitherto by least
squares fitting.

We determine it accurately. The local exponent at a stagnation point is
$\mu = (c-1)/(ac)$, with $c$ the Hilbert transform of $f$ there and
$\beta = \mu+1$, due to Chen and closing as $c = 1/(1-a)$ where $f$ has a
simple zero. Reading $c$ at the other stagnation point is the difficulty:
it wanders by $2\times10^{-2}$ across seven resolutions. Both carry the same
discretisation error and one is known exactly, so regressing one on the other
and evaluating at the exact value cancels it, collapsing the spread by $800$.
That the errors are proportional is tested, not assumed: the error is a single
shape to one part in $10^4$, and inducing it differently moves the corrected
value by $1.5\times10^{-6}$.

At $a=0.8$ this gives $\beta = 3.0024227\pm9.1\times10^{-6}$, excluding
$\beta=3$, and $\beta$ varies continuously with $a$ rather than locking to an
integer. At $a=0.71$ it gives $9.29546$ against the published fitted
$9.32592$, a correction of $0.33$ percent, the two sharing no
machinery.

Three smaller results: the linearisation in renormalised time has
exactly two eigenvalues outside the open left half plane, both forced by
symmetry; the profile equation has a first integral whose closure forces
$\mathrm{PV}\oint \mathrm{d}y/U = 0$, verified to $10^{-12}$; and at $a=1/2$,
$\omega_0 = \sin x$ blows up at $T = \pi$ exactly.
\end{abstract}
...
\item \emph{The control variate, and $\beta$ to five digits.} The two
  ...
  spread by a factor of $800$, giving $\beta = 3.0024227\pm9.1\times10^{-6}$ at
...
\item \emph{A correction to the published exponent.} At $a=0.71$ the same
  ...
  Lushnikov, Silantyev and Siegel \cite{lss2020}, a difference of $0.33$ percent between two methods sharing no
...
\item \emph{The linear spectrum at $a=0.8$.} Exactly two eigenvalues lie
  ...
  away from the neighbourhood of $a=1$ in which Chen \cite{chenS1} proves nonlinear
...
and Section \ref{sec:profile} treats the profile equation as a fixed point of
the evolution nonlinearity and checks the amplitude it fixes against simulation
to $10^{-5}$.
...
What this paper adds is the determination of the constant those results leave
open, and a reason to distrust the way it has been obtained so far. The
exponent $\beta$ has been read off least squares fits to the spectrum, which
Section \ref{sec:control} shows to be accurate only to about a percent and
biased low; the control variate developed there returns five digits from a
single local quantity, fitting nothing, and it works at $a\approx0.8$, away
from the $a\to1^-$ neighbourhood in which \cite{chenS1} controls the profile.

The consequences reach past the decimal places. Spectral fitting is the
standard route to this exponent throughout the literature above, so its bias
attaches to published values and not only to ours: Remark \ref{rem:pb} corrects
one. The corrected exponents vary continuously with $a$ and exclude the integer
that fitting had been unable to resolve, which is a statement about the
structure of these profiles rather than about precision. And the error analysis
of Remark \ref{rem:oneanchor} gives a testable condition, that the
discretisation error be of rank one, under which cancelling one constant
against another is legitimate at all. That condition is checkable wherever the
technique is tried.
...
Okamoto, Sakajo and Wunsch \cite{osw2008} study global existence against blowup
by a Beale, Kato and Majda type criterion and do not consider self similar
profiles, but they do fit the spectrum as
$\abs{\hat\omega_n} \sim Cn^{-p}e^{-\delta n}$ and report that $p(t)$ decreases
monotonically without their being able to see its asymptotic value. The
exponent $\beta$ of \eqref{eq:beta} is the analogue of that $p$ for a frozen
profile, determined in closed form given one local quantity.

The remaining constructions are on the real line and they narrow. Huang, Tong
and Wei \cite{htw2022} build infinitely many compactly supported self similar
solutions as eigenfunctions of a compact operator; Huang, Tong and Wang
\cite{htw2026} treat data with derivative degeneracy, reporting one scale
blowup with regular profiles for $a>0$ against a two scale scenario for
$a\le0$; Huang, Qin, Wang and Wei \cite{hqww2023} prove exact self similar
blowup with interiorly smooth profiles for all $a\le1$; Huang, Qin and Wang
\cite{hqw2024} construct blowups with two collapsing spatial scales; Zheng
\cite{zheng2023} constructs $C^\alpha$ blowup at small $\abs{a\alpha}$; and Guo
and Jiu \cite{gj2025} prove instability of the excited state $-\sin2\theta$ at
$a=1$, which is the steady state analogue of the observation that the higher
members of Proposition \ref{prop:dilation} are not generic attractors.

Two points of contact deserve naming. The self similar profile equation used
throughout that work, $(c_l x + a u)\omega_x = (c_\omega + u_x)\omega$, reduces
to \eqref{eq:profile} at $c_l = 0$, $c_\omega = -1$, and we claim no novelty for
that formulation. And the profiles of \cite{hqww2023} fail to be smooth only at
the boundary of a compact support, where ours is $C^{1,1}$ at an \emph{interior}
point; their analogues of $c_1$ and $c_2$ are bounded as integrals rather than
evaluated, and no closed form appears.

Those two are consistent, and the reason is the pattern. With the exception of
\cite{lss2020} and \cite{slsa2025}, every construction we are aware of works on
the real line with a narrowing profile, $c_l\ne0$. The circle behaves
differently for a reason that is not a matter of taste: there $U$ has zero mean
by construction, so it must change sign, interior stagnation points are forced,
and by Proposition \ref{prop:exponent} the profile cannot be smooth at them.
That is the setting in which a frozen profile, $c_l=0$, becomes the attractor,
and it is the setting \cite{lss2020} already works in.
...
predicted one. The control variate of Section \ref{sec:control}, the first
integral and global constraint of Section \ref{sec:integral}, and the linear
stability computation of Section \ref{sec:spectrum} are likewise not in them.
Two numerical coincidences with \cite{htw2026} invited closer comparison, and
...
are not the same object, since their profiles carry $c_l \ne 0$ and a frozen
profile requires $c_l=0$ exactly, and it is tempting to read the two as
complementary halves of one exchange of stability. Table \ref{tab:cl} half
supports that reading: $c_l$ falls smoothly to zero rather than jumping, which
is the right shape for an exchange. The location does not survive.

We initially read Table \ref{tab:cl} as giving
$c_l = 0.004536\pm0.000543$ at $a=0.751$, eight standard errors from zero, and
concluded $a_c\gtrsim0.795$. That reading is withdrawn. The critical advection
is known
far better than anything measured here, \cite{lss2020} obtaining
$a_c = 0.6890665337007457\ldots$ from a nonlinear eigenvalue problem and
\cite{xu2026} recomputing the branch independently by Newton continuation to
$0.6888$. The failure is one of resolution, visible in Table \ref{tab:cl} once
the published branch is set beside it: $1.058$ against $1.0000$ at $a=0$,
$0.512$ against $0.4809$ at $a=0.4$, $0.325$ against the exact $1/3$ at
$a=0.5$. A half width measurement carrying several percent of bias cannot
resolve a quantity of size $0.0045$, and fitting $c_l = A(a_c-a)^p$ through it
was an extrapolation through convex data of exactly the kind warned against in
Section \ref{sec:control}.

A check from this paper agrees with the published value rather than the
withdrawn one. \cite{lss2020} reports the spectral exponent diverging as
$a\to a_c^+$, and $\beta$ from Section \ref{sec:control} does the same, taking
$16.362$, $9.295$, $6.795$ and $4.213$ at $a=0.70,0.71,0.72,0.75$;
extrapolating $1/\beta$ linearly to zero from the two closest pairs gives
$0.6868$ and $0.6828$, consistent with $a_c=0.68907$ and not with $0.795$. We
quote it loosely, since $1/\beta$ is convex and this is the same trap once
more, but its direction is not in doubt. The coincidence with \cite{htw2026}
therefore stands unresolved rather than refuted: their $a^{c,2}\approx0.751$ is
a stability boundary for a profile on the line, $a_c$ is where the width
exponent vanishes on the circle, and nothing here decides whether they are
related.
...
We first compared Proposition \ref{prop:c1} only against Theorem 2.6 of
\cite{htw2026}, an exact solution for $a<0$ with $\mu = 1/(1-a)$, and concluded
the resemblance was superficial on the grounds that their $\mu$ is an exponent
while ours is a value of $\Hil f$. That comparison was aimed at the wrong part
of the paper: equation (2.10) of the same reference is the relation itself. We
record the error because it is the kind that a reader checking one theorem
rather than the whole paper will repeat.
...
rate exponent one throughout, and with $T$ diverging as $a\to1$ rather than any
transition to global existence at intermediate $a$. Table \ref{tab:times}
collects the times, estimated by fitting
$\omega/(\mathrm{d}\omega/\mathrm{d}t) = (T-t)/p$, a straight line in $t$ with
slope $-1/p$ and root $T$. The value at $a=1/2$ is $\pi$ exactly, by
Proposition \ref{prop:pi}.
...
The blowups are not all alike, and the dichotomy is due to \cite{lss2020}, who
established it on the circle, located the transition at
$a_c = 0.6890665337007457\ldots$, and computed the width exponent to five to
eight digits by a nonlinear eigenvalue solve. We reproduce it by a much cruder
route, reported as a check on our diagnostic rather than as a determination of
theirs. Comparing normalised spectra
$\abs{\hat\omega_k}/\max_k\abs{\hat\omega_k}$ at successive decades of amplitude
growth is invariant under translation and assumes nothing about the profile
shape. At $a=0.8$ the curves converge, successive deviations from the first
snapshot running $3.30$, $3.68$, $3.76$ and $3.78\times10^{-3}$ across four
decades while the peak locks to $x=2.16699$ and the spectral tail stays at
$2.7\times10^{-17}$; at $a=0.4$ the same comparison moves by $0.72$ over a
single decade and resolution is exhausted by
$\norm{\omega}_\infty = 5.8\times10^3$. We call the first a \emph{frozen}
profile and the second \emph{narrowing}, and only the frozen case is analysed
below.

Quantitatively, in the general self similar ansatz of Proposition
\ref{prop:general} the peak width scales like $(T-t)^{c_l}$ and the amplitude
like $(T-t)^{-1}$, so $c_l$ is minus the slope of one against the other on
logarithmic axes and a frozen profile is exactly $c_l=0$. Measuring the width at
half maximum, which is robust whether the spectrum is exponential or algebraic,
gives Table \ref{tab:cl}; the method returns $1.048$ at $a=0$, where
\eqref{eq:clmexact} gives $c_l=1$. Against the published branch it is accurate
to a few percent and not of one sign, and the introduction gives that
comparison term by term together with the estimate of $a_c$ it forces us to
withdraw.
...
the variable $X=\tan(x/2)$ their $a=1/2$ solution is a conjugate pair of one
double and one simple pole with amplitudes tied by
$\omega_{-1} = 2iv\,\omega_{-2}/(1-v^2)$, the constraint that cancels the
logarithmic singularities advection otherwise generates. Our datum lies in
their class, since $\sin x = 1/(X-i)+1/(X+i)$ is a single conjugate pair at
$v=1$: the limit in which the complex singularity sits at infinite height, as
it must for an entire datum, and the one point at which their parametrisation
degenerates, the constraint reading $1 = (2i/0)\cdot0$ there. The degeneracy is
removable, and clearing it gives a specialisation of \cite{slsa2025} rather
than a new solution.
...
for the first time; it is recorded at $a=1/2$ in \cite{lss2020} and restated in
\cite{slsa2025}, and the computation above merely confirms that our datum sits
on the same branch. The total advection velocity is
\begin{equation}
...
\begin{equation}
  c_2 = -1.66130027 \pm 2.0\times10^{-5}, \quad
  \mu_2 = 2.00242268, \quad
  \beta = 3.00242268.
\end{equation}
The quoted uncertainty is set by the corrected spread of $2.4\times10^{-5}$ and
by the split half test below, not by the standard deviation of the corrected
values, which is $1.1\times10^{-5}$. The looser figure is the honest one:
correcting the coarse half of the ladder and the fine half separately, so that
the two share no resolutions, gives $-1.66130963$ and $-1.66129332$, which
differ by $1.6\times10^{-5}$. The fine half interpolates less favourably, its
three values of $c_1$ lying within a window of width $2.2\times10^{-3}$ entirely
below the exact anchor, and the discrepancy is consistent with that. It is in
any case immaterial to the exclusion of $\beta=3$, which stands at $5.4\times
10^{-3}$, more than two hundred times the widest reading of the uncertainty.

An independent pass supports the central value. Seeding every grid from a
single interpolated simulation rather than simulating at each resolution, and
omitting $N=4096$, gives $c_2 = -1.66130020$ and $\beta = 3.00242271$, differing
from the above in the eighth and ninth digits respectively.
...
\begin{remark}[Why the errors are proportional]
\label{rem:oneanchor}
The correction uses a single exactly known quantity, so the hypothesis that the
two errors are proportional deserves a test independent of the fit that assumes
it. It has one. Solving the profile at $N=512$ to $3072$, interpolating each
solution to a common grid and translating each so that $y_1$ coincides, the
differences from the finest solution have singular values
\begin{equation}
  9.970\times10^{-1},\quad 4.699\times10^{-4},\quad 3.788\times10^{-5},\quad
  8.850\times10^{-6},\quad 2.339\times10^{-6},
\end{equation}
so the leading mode carries $0.9999998$ of the variance. The alignment is
necessary rather than cosmetic: a relative translation is a large mode to which
neither $c_1$ nor $c_2$ is sensitive, and retaining it would guarantee the
conclusion. The discretisation error is, to a part in $10^{4}$, a single fixed
shape whose amplitude alone depends on $N$. Every linear functional of it is
then proportional to every other across the ladder, which is the hypothesis,
and it holds for reasons independent of which two constants are read.

The same statement predicts that the correction should not depend on how the
error is induced. Holding $N=2048$ and varying the dealiasing fraction from
$0.20$ to $1/3$, which changes the retained band without changing the grid,
gives a regression of slope $-0.982306$ against $-0.980354$ for the resolution
ladder, a genuinely different perturbation, and corrected values differing by
$1.5\times10^{-6}$.

What remains absent is agreement with a determination that does not pass
through this discrete operator, and we know of none. Both tests above vary the
discretisation within one family, so a systematic common to the whole family
would survive them.
\end{remark}
...
\paragraph{That verification is weaker than it looks.} For the profile studied
here \eqref{eq:pv} holds by symmetry, independently of the profile equation.
Section \ref{sec:stagnation} records that $f$ is odd about $y_1$; hence
$\Hil f$ is even about $y_1$, hence $U$ is odd about it, measured as
$\max\abs{U(y_1+s)+U(y_1-s)}/\norm{U}_\infty = 3.7\times10^{-14}$ at $a=0.8$,
$N=2048$. An odd integrand has vanishing principal value on the circle by
antisymmetry alone. Consistently, the computed value of
$\mathrm{PV}\oint\mathrm{d}y/U$ does not improve with resolution: across
$N=512$ to $3072$ at $a=0.8$ it wanders between $8.4\times10^{-14}$ and
$8.9\times10^{-13}$ with no trend and changing sign, while over the same ladder
the error in $c_1$ falls monotonically from $1.3\times10^{-2}$ to
$2.2\times10^{-3}$. So the quantity carries no discretisation error and its
agreement with zero measures the symmetry of the datum rather than the accuracy
of the solve. Proposition \ref{prop:pv} is not thereby weakened as a statement,
since it assumes no symmetry and would have content for a profile that is not
odd, but it is untested by this computation, and it cannot serve as an
independent anchor for the control variate of Section \ref{sec:control}.

\paragraph{Relation to the orbit invariants at $a=1$.} A principal value
integral of this shape is not new to the model. At $a=1$, where \eqref{eq:osw}
becomes the Lie bracket form $\omega_t + [u,\omega]=0$ and the dynamics is
motion on an adjoint orbit of $\mathrm{Diff}_+(S^1)$, Jia, Stewart and
\v{S}ver\'{a}k \cite{jss2019} identify
$b = \mathrm{p.v.}\int_{S^1}\mathrm{d}\theta/\omega_0(\theta)$ as an orbit
invariant in their equation (2.14), and observe that its conservation, together
with the conservation of $\omega_\theta$ at the zeros of $\omega$, is an
analogue of the Kelvin, Helmholtz law. Equation \eqref{eq:pv} is a different
statement: the integrand carries the velocity rather than the vorticity, the
setting is the frozen profile at $a\approx0.8$ rather than the $a=1$ dynamics,
and the content is that the integral vanishes rather than that it is conserved.
The orbit structure they use is special to $a=1$, the only member of the family
that is a pure Lie bracket, and does not survive $a\ne1$. The two are
nonetheless close enough in shape that we claim nothing for the form of
\eqref{eq:pv}, only for its value and for its derivation from
\eqref{eq:logderiv}. We record separately that the conserved quantity of Lei,
Liu and Ren \cite{llr2020}, the $H^1$ norm of $\sqrt{\omega}$ for one signed
data, is unrelated to either.
...
$a=1$ model. Table \ref{tab:generic} answers the worst form of this objection,
reaching the same profile from sixteen random data and from a continuous family
of offsets, always with the same constants to seven digits. It does not map the
basin. A probe of that boundary along two one parameter families, which finds a
window of data decaying like $1/t$ rather than blowing up, is not needed for
the determination of $\beta$ and is left in the repository accompanying this
paper.
...
verified, and twelve closely related works have been read for overlap:
\cite{osw2008}, \cite{htw2022}, \cite{hqww2023}, \cite{hqw2024},
\cite{zheng2023}, \cite{gj2025}, \cite{htw2026}, \cite{lss2020},
\cite{slsa2025}, \cite{xu2026}, \cite{chenS1} and \cite{alss2024}. The setting
is one other people built, and we claim none of the following. The two regime
picture on the circle, the
...
integral and global constraint of Section \ref{sec:integral}, with the
qualification recorded there. We did not find either in \cite{chenS1},
\cite{htw2026} or the pole dynamics papers, but a principal value integral of a
reciprocal with simple zeros, serving as a global invariant, is already in
\cite{jss2019} at $a=1$; \eqref{eq:pv} concerns a different field in a
different setting and says the integral vanishes rather than that it is
conserved, but we claim nothing for its shape. Both are elementary consequences
of separating \eqref{eq:logderiv} and we would not be surprised to be shown
they are known.
...
\cite{xu2026} and \cite{alss2024} belong to a line of work using complex
...
\cite{chenS1} was found later still, through a citation inside \cite{alss2024}
...
The critical limit $a\to1$ is left open and is not treated here. We can report
no exponent for $T(\varepsilon)$, since the measured one does not converge over
the range we can reach, and the shortfall cannot be made up by analysis in
place of computation: the linearisation at $a=1$ has purely imaginary spectrum
with the equilibrium manifold in its kernel, so there is no spectral gap to
reduce onto. That computation is independent of everything above and is left in
...
Every number reported here is reproducible from source. The solver, the profile
solver, the script behind each table and figure, and a suite of nineteen checks
against closed forms and conserved quantities are openly available under an MIT
licence at
\begin{center}
\url{https://github.com/munawarkazmi/degregorio-blowup}
\end{center}
where the checks run on every commit. The two computations referred to above as
left in the repository, the basin probe and the critical limit, are there under
\texttt{basin.py} and \texttt{critical.py}.
...
\bibitem{alss2024} D. M. Ambrose, P. M. Lushnikov, M. Siegel and
  ...
  line versus periodic domains}, Nonlinearity \textbf{37} (2024), 025004.
