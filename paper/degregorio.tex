...
under active study and the present results must be read against it. Huang, Tong
and Wei \cite{htw2022} construct infinitely many compactly supported self
similar blowup solutions of the De Gregorio model on the real line, organised
as eigenfunctions of a compact operator and as solutions of nonlinear
eigenvalue problems. Huang, Tong and Wang \cite{htw2026} study self similar
blowup of the generalised model from data with derivative degeneracy, and
report one scale blowup with regular profiles for $a>0$ against a two scale
scenario with a singular outer profile for $a\le 0$, constructing an explicit
family of singular profiles in the latter regime.

Our setting differs in domain, in datum and in parameter range: we work on the
circle rather than the line, from $\omega_0 = \sin x$ rather than degenerate
data, and in the window $0.7 \lesssim a \lesssim 0.85$. The fixed point
formulation \eqref{eq:profile} is the standard self similar profile equation
and we claim no novelty for it. What we have not been able to establish is
whether the local relation of Proposition \ref{prop:exponent}, and in
particular the closed form of Proposition \ref{prop:c1}, appear in that
literature. They are elementary enough that they may well be known, and this
draft should not be circulated until that has been checked directly against
\cite{osw2008}, \cite{htw2022} and \cite{htw2026} rather than against their
abstracts.
...
Priority is unestablished. The bibliographic details of every reference above
have been verified, but the results themselves have not been checked against
the contents of those papers, only against their abstracts. Given
\cite{htw2022} and \cite{htw2026}, the possibility that some or all of Sections
\ref{sec:regimes} to \ref{sec:stagnation} is known must be treated as likely
rather than remote until checked.
