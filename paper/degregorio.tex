...
\begin{remark}[The correction rests on one anchor]
\label{rem:oneanchor}
The correction uses a single exactly known quantity, and we were unable to
validate it against a second. The only other constant available in closed form
on this profile is the constraint of Section \ref{sec:integral}, and that one
holds by symmetry rather than by accuracy, so it carries no discretisation
error and cannot anchor anything. The evidence that the correction is doing
what it claims is therefore internal: the corrected values from six resolutions
collapse onto one number, the raw spread falling by a factor of $800$, and
$N=512$ lands within $1.5\times10^{-5}$ of $N=4096$ once corrected. That is
consistency across resolutions, not agreement with an independent
determination, and it does not exclude a systematic common to every
resolution. A second anchor, in this problem or another, would be the natural
next test.
\end{remark}
