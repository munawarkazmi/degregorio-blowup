...
Table \ref{tab:cl} tests that reading and does not support it. The mechanism
survives: $c_l$ falls smoothly to zero rather than jumping, so the narrowing
branch merges with the frozen one rather than being replaced by it, which is
consistent with an exchange. The \emph{location} does not. Where $c_l$ reaches
zero cannot be pinned from these data: fitting $c_l \sim A(a_c-a)^p$ over the
last resolved points is degenerate, admitting $(a_c,p)$ near $(0.79, 2.1)$,
$(0.80, 2.6)$ and $(0.82, 3.3)$ with comparable quality, and by $a=0.78$ the
measurement is already at its noise floor. A linear extrapolation returns
$0.775$, agreeably close to $0.751$, but $c_l(a)$ is convex and a straight line
through convex data underestimates its root, which is the same trap that
produced a spurious spectral exponent in Section \ref{sec:control}.

We therefore withdraw the numerical coincidence. What can be said is that a
branch merger occurs somewhere in $0.78 \lesssim a_c \lesssim 0.82$, that this
is the right kind of event to pair with the instability reported in
\cite{htw2026}, and that the present measurement cannot distinguish that
interval from $0.751$ sharply enough to claim they are the same. Settling it
needs their profile computed on the circle, which we have not done.
...
\toprule
$a$ & $0$ & $0.2$ & $0.4$ & $0.5$ & $0.6$ & $0.7$ & $0.75$ & $0.8$\\
\midrule
$c_l$ & $1.066$ & $0.748$ & $0.512$ & $0.325$ & $0.179$ & $0.040$
      & $0.0068$ & $0.0012$\\
\bottomrule
\end{tabular}
\caption{The spatial scaling exponent measured from the dynamics. The exact
value at $a=0$ is $1$. Beyond $a\approx 0.78$ the measurement is at its noise
floor of about $10^{-3}$ and $c_l$ is indistinguishable from zero.}
\label{tab:cl}
\end{table}
