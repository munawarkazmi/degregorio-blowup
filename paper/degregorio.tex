...
Majda model at $a=0$ and the De Gregorio model at $a=1$. On the circle,
Lushnikov, Silantyev and Siegel identified two regimes separated by a critical
advection $a_c = 0.6890665\ldots$: below it the blowup narrows, while above it
the solution converges to a fixed profile $f$ with $\omega = f(x)/(T-t)$, whose
Fourier spectrum decays algebraically because a high order derivative of $f$
jumps at the point antipodal to the singularity. They obtained the exponent of
that decay by fitting the spectrum. This paper supplies a local theory for the
frozen profile which predicts the exponent instead of fitting it.

Rearranging the profile equation shows that $f$ can be singular only where its
own velocity vanishes. There are exactly two such stagnation points, they are
exactly $\pi$ apart, which we show is forced by a parity argument, and at each
the local exponent is $\mu = (c-1)/(ac)$ where $c$ is the Hilbert transform of
$f$ at that point. Since $\abs{y}^\mu$ has Fourier coefficients decaying like
$k^{-(\mu+1)}$, one local number therefore predicts the entire spectral decay
rate $\beta = \mu+1$. At the stagnation point where $f$ has a simple zero this
closes in elementary form as $c = 1/(1-a)$, verified to machine precision; the
general self similar version, $h = (1+c_l)/(1-a)$, also appears in recent work
of Xu, and an equivalent balance gives the complex singularity exponent of
Lushnikov, Silantyev and Siegel. The other stagnation point is the one that
carries the singularity, and its $c$ is genuinely global: no local balance
determines it. Using the exact value at the first point as a control variate
removes the discretisation error at the second, giving five digits or better.
We conclude that $\beta$ varies continuously with $a$ and is not equal to $3$,
with $\beta = 3.0024227$ at $a=0.8$, and that at $a=0.71$ the predicted
$\beta = 9.2955$ sits $0.33$ percent below the published fitted value
$9.3259$.

We add three further results. The linearisation about the profile, taken in
renormalised time, has exactly two eigenvalues outside the open left half
plane, both forced by symmetry, so the profile is linearly stable modulo those
symmetries, which sharpens the nonlinear stability previously inferred from
convergence in simulations. The profile satisfies a fixed point equation
$\mathcal{R}(f)=f$ admitting no free scaling, so it determines its own
amplitude, and that amplitude agrees with the simulation to $10^{-5}$. Finally
we give an exact first integral of the profile equation and a global
constraint, $\mathrm{PV}\oint \mathrm{d}y/U = 0$, verified to $10^{-12}$, and
we record that at $a=1/2$, where the model is exactly solvable, the datum
...
What this paper adds to that is the local theory. Their $p_b$ comes from a
least squares fit to the spectrum over a window; ours follows from
\eqref{eq:beta} given a single value of $\Hil f$, and Section
\ref{sec:control} shows that the two differ in the third digit.
...
The same factor appears twice more in the pole dynamics literature, and once by
the same computation. Theorem 1 of \cite{lss2020} gives $\gamma = 1/(1-a)$ for
the power of the leading complex singularity of $\omega$ off the real axis,
from the balance $a\gamma/(\gamma-1) = 1$, that is $a\gamma = \gamma-1$, which
is identical in form to the $ac = c-1$ of Proposition \ref{prop:c1}. The
objects are different, theirs a power at a complex singularity and ours a value
of $\Hil f$ at a real stagnation point, but the balance of stretching against
advection which produces the factor is the same, and we make no priority claim
for it. More directly, \cite{xu2026} states the general form
$\Hil\Omega(0) = (1+c_l)/(1-a)$ outright, which is Proposition
\ref{prop:general}, of which Proposition \ref{prop:c1} is the $c_l=0$ case.

Our contributions are as follows.

\begin{enumerate}
\item We recover the two regime picture of \cite{lss2020} on the circle with an
  independent and translation invariant diagnostic, and record where our
  resolution falls short of theirs (Section \ref{sec:regimes}).
\item We formulate the frozen profile equation as a fixed point of the
  evolution nonlinearity, observe that it admits no free scaling and therefore
  fixes the profile amplitude outright, and verify that prediction against
  simulation to $10^{-5}$ (Section \ref{sec:profile}).
\item We compute the spectrum of the linearisation about the frozen profile in
  renormalised time and identify the two symmetry forced eigenvalues exactly,
  one of which is provable in closed form, which sharpens the nonlinear
  stability inferred in \cite{lss2020} from observed convergence (Section
  \ref{sec:spectrum}).
\item We prove that singularities of the profile occur only at stagnation
  points of its velocity, that there are exactly two and that they are exactly
  $\pi$ apart, and we derive the local exponent $\mu = (c-1)/(ac)$ there. The
  closed form $c = 1/(1-a)$ at the stagnation point where $f$ is smooth is the
  $c_l=0$ case of a relation also given in \cite{xu2026}; the exponent at the
  \emph{singular} stagnation point, where $\mu\ne1$, is the one the rest of the
  paper rests on (Section \ref{sec:stagnation}).
\item We determine the remaining, global constant to five digits using the
  exact relation as a control variate, concluding that the spectral decay
  exponent varies continuously with $a$, that it is not equal to $3$, and that
  at $a=0.71$ it sits $0.33$ percent below the value obtained in
  \cite{lss2020} by fitting the spectrum (Section \ref{sec:control}).
\item We test the $c_l$ dependence of Proposition \ref{prop:general} against
  the exact $a=1/2$ solution of \cite{slsa2025}, and record that the datum
  $\omega_0 = \sin x$ blows up there at $T=\pi$ exactly (Section
  \ref{sec:exacthalf}).
...
verified. Ten closely related works, \cite{osw2008}, \cite{htw2022},
\cite{hqww2023}, \cite{hqw2024}, \cite{zheng2023}, \cite{gj2025},
\cite{htw2026}, \cite{lss2020}, \cite{slsa2025} and \cite{xu2026}, have been
read for overlap, and the honest summary is that this paper is a local theory
laid on top of a setting which \cite{lss2020} established.

We claim none of the following, all of which are in \cite{lss2020}: the two
regime picture on the circle, the critical advection $a_c$, the frozen blowup
$\omega = f(x)/(T-t)$ and its numerical profile, the derivative jump at the
antipodal point, the algebraic spectral decay it causes, and the nonlinear
stability of the profile as observed from converging simulations. Equation
\eqref{eq:hgeneral} and hence Proposition \ref{prop:general} are in
\cite{xu2026}. We derived the relation independently and became aware of
\cite{xu2026} only afterwards, which is a statement about how the work went and
not a claim on it: the relation is theirs, and we state it here only because
Proposition \ref{prop:exponent} is the case it does not cover. Proposition
\ref{prop:c1} is its $c_l=0$ case, and shares its algebra with Theorem 1 of
\cite{lss2020}.
The exact $a=1/2$ solution used in Section \ref{sec:exacthalf} is from
\cite{slsa2025} and Proposition \ref{prop:pi} is a specialisation of it, claimed
only as such.

What we do claim, and did not find in any of them, is Proposition
\ref{prop:exponent} at the stagnation point where the zero of $f$ is not
simple, which converts the fitted exponent of \cite{lss2020} into a predicted
one; the parity argument fixing the two stagnation points exactly $\pi$ apart;
the control variate determination of $c_2$ and the resulting exclusion of
$\beta=3$, together with the correction in Remark \ref{rem:pb}; the spectrum of
the linearisation about the frozen profile on the circle; and the first
integral and global constraint of Section \ref{sec:integral}.

This list grew three times while being compiled, most consequentially at the
end, and that history is itself a caveat. \cite{lss2020}, \cite{slsa2025} and
\cite{xu2026} belong to a line of work using complex singularity and pole
dynamics methods which searches organised around the other references do not
surface, and it is the line which works on the circle, the setting this paper
relies on. We have read \cite{lss2020} in full and \cite{xu2026} in part, and
we expect that literature to bear further on Sections \ref{sec:control} and
\ref{sec:exacthalf}. For the preprints this rests on a reading of the text
rather than a line by line verification of every lemma. Both numerical
coincidences with \cite{htw2026} have been examined; neither is an overlap, and
the first, the exchange of stability near $a\approx0.75$, is now to be compared
against $a_c = 0.68907$ rather than against the withdrawn estimate, and remains
unsettled.
...
\bibitem{lss2020} P. M. Lushnikov, D. A. Silantyev and M. Siegel,
  \emph{Collapse versus blow up and global existence in the generalized
  Constantin, Lax and Majda equation}, preprint, arXiv:2010.01201 (2021).
...
\bibitem{slsa2025} D. A. Silantyev, P. M. Lushnikov, M. Siegel and
  ...
  Stud. Appl. Math. (2025); preprint, arXiv:2411.01891.
