...
\usepackage{lmodern}
% headheight and headsep are zeroed because the header is empty; left at their
% defaults, fancyhdr reserves about 1.3cm above the text block and the title
% sits well down the first page.
\usepackage[margin=3.5cm,top=2.8cm,headheight=0pt,headsep=0pt]{geometry}
\usepackage{xcolor}
\usepackage{graphicx}
\usepackage{amsmath}
...
\begin{center}
  ...
  {\normalsize Munawar Kazmi \quad$\bullet$\quad
  \href{https://munawarkazmi.com}{munawarkazmi.com}}\\[6pt]
  ...
  \href{https://github.com/munawarkazmi/degregorio-blowup}%
...
happens for every setting below one. The moment it happens gets later and later
as the dial is turned up, but it always arrives.

\begin{center}
\begin{tabular}{lcccccccc}
\hline
dial setting & 0 & 0.4 & 0.6 & 0.8 & 0.9 & 0.95 & 0.98 & 0.99\\
becomes infinite at & 2.00 & 2.79 & 3.65 & 6.07 & 10.76 & 17.71 & 32.18
& 56.97\\
\hline
\end{tabular}
\end{center}

The first column is the 1985 answer, known exactly. The last is a computation
that took nearly a million steps. Push the dial closer to one and the time
keeps growing, without ever becoming infinite until the dial reaches one
exactly.
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{fig_kink.png}
  \caption{On the left, the shape the wave settles into, with the point where
  the flow is at rest marked. Nothing about it looks wrong. On the right, the
  curvature of that same shape near that same point, which does not pass
  through zero but leaps across it. The small ripples either side are an
  artefact of measuring curvature, not part of the shape.}
\end{figure}
...
withdrawn.\\
Repository: \href{https://github.com/munawarkazmi/degregorio-blowup}%
{github.com/munawarkazmi/degregorio-blowup}}
